#include <QStringList>
#include <QtPlugin>
#include <QSettings>
#include <QAction>

#include "projectexplorer/projectmanager.h"
#include "projectexplorer/peprojectversion.h"
#include "core/core_constants.h"
#include "core/actionmanager.h"
#include "core/actioncontainer.h"
#include "core/command.h"
#include "core/mainwindow.h"

#include "baycopyplugin.h"
#include "baycopy_constants.h"
#include "copybaydlg.h"

using namespace BayCopy::Internal;

BayCopyPlugin::BayCopyPlugin()
{
}

BayCopyPlugin::~BayCopyPlugin()
{
}

bool BayCopyPlugin::initialize(const QStringList &arguments, QString *errorMessage)
{
    Q_UNUSED(arguments)
    Q_UNUSED(errorMessage)

    return true;
}

void BayCopyPlugin::extensionsInitialized()
{
    SlotReadSettings();

    //connect(Core::ICore::instance(), SIGNAL(sigCoreAboutToClose()),
    //        this, SLOT(SlotWriteSettings()));    

    QList<int> lstGlobalContexts = QList<int>() << Core::Constants::g_iUID_Context_Global;

    Core::CActionManager *pActionManager = Core::MainWindow::Instance()->GetActionManager();
    Core::CActionContainer *pActionContainerMenu = pActionManager->GetActionContainer(Core::Constants::g_szUID_Menu);

    QAction *pActionToolsCopyBay = new QAction(tr("&Bay Copy..."), this);
    Core::CCommand *pCommand = pActionManager->RegisterAction(pActionToolsCopyBay, QLatin1String("BayCopy.Action.ToolsCopyBay"), lstGlobalContexts);
    pCommand->SetDefaultKeySequence(QKeySequence("Ctrl+B"));
    pActionContainerMenu->AddAction(pCommand, Core::Constants::g_szUID_MenuGroup_Tools);
    connect(pActionToolsCopyBay, SIGNAL(triggered()), this, SLOT(SlotActionCopyBay()));
}

void BayCopyPlugin::aboutToShutdown()
{
}

void BayCopyPlugin::SlotReadSettings()
{
    /*QSettings *pSettings = Core::ICore::instance()->GetSettings();

    pSettings->beginGroup(Config::Constants::g_szSettings_Group_Config);
    m_pProjectMode->ReadSettings(pSettings);
    pSettings->endGroup();*/
}

void BayCopyPlugin::SlotWriteSettings()
{
    /*QSettings *pSettings = Core::ICore::instance()->GetSettings();

    pSettings->beginGroup(Config::Constants::g_szSettings_Group_Config);
    m_pProjectMode->WriteSettings(pSettings);
    pSettings->endGroup();*/
}

void BayCopyPlugin::SlotActionCopyBay()
{
    CopyBayDlg dlg(Core::MainWindow::Instance());
    dlg.exec();
}

Q_EXPORT_PLUGIN(BayCopyPlugin)
